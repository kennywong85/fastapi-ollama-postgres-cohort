...
if (-not ($dbList -match 'llm_question_log')) {
    Fail "Database 'llm_question_log' not found. Create it: createdb -U postgres llm_question_log"
}
Ok "Database llm_question_log exists"

# 6. Table interactions exists.
$tableExists = & psql -U postgres -d llm_question_log -tAc `
    "SELECT 1 FROM information_schema.tables WHERE table_name='interactions'" 2>$null
if ($tableExists.Trim() -ne '1') {
    Fail "Table 'interactions' not found. Apply schema: psql -U postgres -d llm_question_log -f sql/001_create_interactions.sql"
}
